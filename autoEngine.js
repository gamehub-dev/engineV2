...
// classes
class Engine_Player {
    constructor(x, y, vy, vxl, vxr, w, h, s, j, f, g, c, i, sob, jumping) {
        this.x = x;
        this.y = y;
        this.vy = vy;
        this.vxl = vxl;
...
let enginePlayer = new Engine_Player(100, 100, 0, 0, 0, 50, 50, 5, 15, 0.2, false, "black", false, false, false);
...
class Engine {
    constructor(p, g, b, bc, ox, oy, o, rc, c, is, tb, tt, tl, tr, bw, bbc, i, bcc, up) {
...
let engine = new Engine(false, false, false, false, false, false, false, false, false, '', 0, 0, 0, 0, 2, 'black', 0, 'black', false)
...
function engineCollision() {
        if (!imgMovement) {
            ...
            for (let i = 0; i < engineBlocks.length; i++) {
                ...
                if (enginePlayer.x <= engineBlocks.at(i).x + engineBlocks.at(i).w && enginePlayer.x + enginePlayer.w > engineBlocks.at(i).x && enginePlayer.y + 40 > engineBlocks.at(i).y && enginePlayer.y < engineBlocks.at(i).y + engineBlocks.at(i).h) { enginePlayer.x = engineBlocks.at(i).x + engineBlocks.at(i).w };
...
function AllowPlayerMovement() {
    if (engine.playerMovement) {
        ...
        if (engineBlocks.length >= 0){
            engineCollision()
            for (let i = 0; i < engineBlocks.length; i++){
                if (engineBlocks.at(i).color != undefined){ ctx.fillStyle = engineBlocks.at(i).color, ctx.fillRect(engineBlocks.at(i).x, engineBlocks.at(i).y, engineBlocks.at(i).w, engineBlocks.at(i).h);}else{
                    ctx.fillStyle = 'black'
...
listen("keydown", function (e) {
    if (engine.playerMovement) {
        if (engine.gravity) {
            if (enginePlayer.g) {
                if (e.code == 'KeyW') enginePlayer.vy = -enginePlayer.jumpSpeed, enginePlayer.g = false, enginePlayer.jumping = true;
            }
            if (e.code == 'KeyD') enginePlayer.vxr = enginePlayer.speed;
            if (e.code == 'KeyA') enginePlayer.vxl = -enginePlayer.speed;
        }
...
listen("keyup", function (e) {
    if (engine.playerMovement) {
        if (engine.gravity) {
            if (e.code == 'KeyW') enginePlayer.jumping = false;
            if (e.code == 'KeyD') enginePlayer.vxr = 0;
            if (e.code == 'KeyA') enginePlayer.vxl = 0;
        }
    }
